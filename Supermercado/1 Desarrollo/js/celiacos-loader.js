...
const rutaPDF = 'Recursos/BD/CELIACOS L2.pdf'; // archivo fijo

(async function () {
  const imagenDefault = 'images/Celiacos.jpg';
    ...
    productos.forEach((p, idx) => {
      ...
      li.innerHTML = `
        ...
        <div class="astra-shop-summary-wrap">
          <span class="ast-woo-product-category">Celíacos</span>
...
async function leerPDF() {
  try {
    ...
    const inicio = texto.indexOf('CELÍACOS');
    if (inicio > -1) {
      texto = texto.slice(inicio);
    }

        // Eliminar "LIMPIEZA DESBUL" si aparece en la primera línea
    texto = texto.replace(/^CELÍACOS\s+DES\s+BUL\s*/i, '');
